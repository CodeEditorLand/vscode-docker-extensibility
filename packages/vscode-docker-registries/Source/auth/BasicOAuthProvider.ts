...
import { httpRequest, RequestLike } from "../utils/httpRequest";
import { BasicAuthProvider } from "./BasicAuthProvider";

export class BasicOAuthProvider
	...
	implements AuthenticationProvider
{
	...
	public async getSession(
		...
		if (
			...
			const oAuthResponse = await httpRequest<{ token: string }>(
				this.oAuthEndpoint.toString(),
				request,
			);
	...
	public fallback(wwwAuthenticateHeader: string): void {
		const wwwAuthenticateHeaderRegex =
			/Bearer\s+realm="(?<realm>[^"]+)",\s*service="(?<service>[^"]+)",\s*scope="(?<scope>[^"]+)"/i;
		...
		if (
			...
			throw new Error(
				vscode.l10n.t(
					'Unable to parse WWW-Authenticate header: "{0}"',
...
export function isBasicOAuthProvider(
	maybeProvider: unknown,
): maybeProvider is BasicOAuthProvider {
