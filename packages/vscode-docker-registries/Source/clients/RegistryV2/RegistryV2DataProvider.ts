...
export abstract class RegistryV2DataProvider extends CommonRegistryDataProvider {
		...
		do {
			const catalogResponse = await registryV2Request<{
				...
				authenticationProvider:
					this.getAuthenticationProvider(registry),
				method: "GET",
				requestUri: nextLink,
			...
			nextLink = getNextLinkFromHeaders(
	...
	public getLoginInformation(
		...
		throw new Error(
			vscode.l10n.t(
				"Authentication provider {0} does not support getting login information.",
				authenticationProvider,
			),
		);
	}
	...
	public async getManifestV1(item: V2Tag): Promise<Manifest> {
		...
		if (!response.body) {
			throw new Error(
				vscode.l10n.t(
					"Could not find manifest for tag {0}",
					item.label,
				),
			);
		}
		...
		try {
			response.body.history?.forEach((history) => {
				history.v1Compatibility = JSON.parse(
					(history.v1Compatibility as unknown as string) || "{}",
				);
			});
		} catch {
			// Best effort
	...
	protected abstract getAuthenticationProvider(
		item: V2RegistryItem,
