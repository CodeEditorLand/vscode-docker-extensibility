...
import { HeadersLike, httpRequest, RequestLike } from "../../utils/httpRequest";
...
export async function registryV2Request<T>(
	options: RegistryV2RequestOptions,
): Promise<RegistryV2Response<T>> {
	if (
		isBasicOAuthProvider(options.authenticationProvider) &&
		!options.authenticationProvider.didFallback
	) {
		const result = await registryV2RequestInternal<T>({
			...options,
		...
		if (result.succeeded) {
			...
			options.authenticationProvider.fallback(
				result.headers.get("www-authenticate") as string,
			);
		} else {
			throw new HttpErrorResponse(
				options.requestUri.toString(),
				result.status,
				result.statusText,
			);
		}
...
async function registryV2RequestInternal<T>(
	options: RegistryV2RequestOptions,
): Promise<RegistryV2Response<T>> {
