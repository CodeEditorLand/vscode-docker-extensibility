...
import {
	RegistryWizardSecretPromptStep,
	RegistryWizardUsernamePromptStep,
...
import {
	...
	V2RegistryItem,
	V2RegistryRoot,
} from "../RegistryV2/RegistryV2DataProvider";
import {
	GenericRegistryV2WizardContext,
	GenericRegistryV2WizardPromptStep,
...
export function isGenericV2Registry(item: unknown): item is V2Registry {
	return (
		isRegistry(item) &&
		isContextValueRegistryItem(item) &&
		item.additionalContextValues?.includes(GenericV2ContextValue) === true
	);
}
...
export class GenericRegistryV2DataProvider extends RegistryV2DataProvider {
	...
	private readonly authenticationProviders = new Map<
		string,
		BasicOAuthProvider
	>(); // The tree items are too short-lived to store the associated auth provider so keep a cache
	...
	public async getRegistries(
		root: V2RegistryRoot | V2RegistryItem,
	): Promise<V2Registry[]> {
	...
	protected override getAuthenticationProvider(
		item: V2Registry,
	): BasicOAuthProvider {
		const registry = item.baseUrl.toString();

		if (!this.authenticationProviders.has(registry)) {
			const provider = new BasicOAuthProvider(
	...
	public async addTrackedRegistry(): Promise<void> {
		const wizardContext: GenericRegistryV2WizardContext = {
			...
			secretPrompt: vscode.l10n.t(
				"Registry Password or Personal Access Token",
			),
			connectedRegistries: this.authenticationProviders
				? [...this.authenticationProviders.keys()]
				: [],
		};

		const wizard = new RegistryWizard(wizardContext, [
		...
		const registryUriString = wizardContext.registryUri
			.toString()
			.toLowerCase();
		...
		const authProvider = new BasicOAuthProvider(
			this.extensionContext.globalState,
			this.extensionContext.secrets,
