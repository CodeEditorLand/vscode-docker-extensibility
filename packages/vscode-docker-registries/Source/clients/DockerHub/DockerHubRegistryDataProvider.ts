...
import {
	BasicCredentials,
	LoginInformation,
} from "../../contracts/BasicCredentials";
...
export function isDockerHubRegistry(item: unknown): item is CommonRegistry {
	return (
		...
		item.additionalContextValues?.includes(DockerHubContextValue) === true
	);
}
...
export class DockerHubRegistryDataProvider extends CommonRegistryDataProvider {
	...
	public async onConnect(): Promise<void> {
		const wizardContext: RegistryWizardContext = {
			...
			secretPrompt: vscode.l10n.t(
				"Docker Hub Password or Personal Access Token (requires `Read & Write` permissions)",
			),
		};
	...
	public async getRegistries(
		root: CommonRegistryRoot,
	): Promise<CommonRegistry[]> {
		const orgsAndNamespaces = new Set<string>();

		(await this.getOrganizations()).forEach((org) =>
			orgsAndNamespaces.add(org),
		);
		(await this.getNamespaces()).forEach((namespace) =>
			orgsAndNamespaces.add(namespace),
		);
