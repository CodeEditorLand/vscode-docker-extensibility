...
import {
	RegistryWizardRequiredUsernamePromptStep,
...
export function isGitHubRegistry(item: unknown): item is V2Registry {
	return (
		...
		item.additionalContextValues?.includes(GitHubContextValue) === true
	);
}
...
export class GitHubRegistryDataProvider extends RegistryV2DataProvider {
	...
	public constructor(
		...
		this.authenticationProvider = new BasicOAuthProvider(
	...
	public async onConnect(): Promise<void> {
		const wizardContext: RegistryWizardContext = {
			...
			secretPrompt: vscode.l10n.t(
				"GitHub Personal Access Token (requires `repo` and `write:packages` scopes)",
			),
		};
	...
	public override async getRegistries(
		root: V2RegistryRoot,
	): Promise<V2Registry[]> {
	...
	public override async getRepositories(
		registry: V2Registry,
	...
	protected getAuthenticationProvider(
		item: V2RegistryItem,
	): AuthenticationProvider<never> {
		return this.authenticationProvider;
	}

	protected override async getTagCreatedDate(
		item: V2Tag,
		...
		if (digest) {
			const configFile = await registryV2Request<{ created: string }>({
				authenticationProvider:
					this.getAuthenticationProvider(repository),
				method: "GET",
				requestUri: digestRequestUrl,
				scopes: [`repository:${repository.label}:pull`],
			});

			return configFile.body?.created
				? new Date(configFile.body.created)
				: undefined;
		}
