...
export abstract class RegistryWizardPromptStep<
 ...
 */
export class RegistryWizardUsernamePromptStep<
	T extends RegistryWizardContext,
> extends RegistryWizardPromptStep<T> {
	public async prompt(wizardContext: T): Promise<void> {
		wizardContext.username = await showInputBox({
			isSecretStep: false,
			prompt:
				wizardContext.usernamePrompt ||
				l10n.t("Enter your username, or press 'Enter' for none"),
		});
	}
 ...
 */
export class RegistryWizardSecretPromptStep<
	T extends RegistryWizardContext,
> extends RegistryWizardPromptStep<T> {
	public async prompt(wizardContext: T): Promise<void> {
		wizardContext.secret = await showInputBox({
			isSecretStep: true,
			prompt: wizardContext.secretPrompt || l10n.t("Enter your password"),
		});
	}
 ...
 */
export class RegistryWizardRequiredUsernamePromptStep<
	T extends RegistryWizardContext,
> extends RegistryWizardUsernamePromptStep<T> {
	public override async prompt(wizardContext: T): Promise<void> {
		const options: RegistryWizardPromptStepOptions = {
			isSecretStep: false,
			prompt:
				wizardContext.usernamePrompt || l10n.t("Enter your username"),
