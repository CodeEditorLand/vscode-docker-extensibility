...
function isDockerInspectImageConfig(
	maybeImageConfig: unknown,
): maybeImageConfig is DockerInspectImageConfig {
	...
	if (
		...
		typeof imageConfig.ExposedPorts !== "object"
	) {
		return false;
	}
	...
	if (
		...
		!Array.isArray(imageConfig.Entrypoint) &&
		typeof imageConfig.Entrypoint !== "string"
	) {
		return false;
	}
	...
	if (
		imageConfig.Cmd &&
		!Array.isArray(imageConfig.Cmd) &&
		typeof imageConfig.Cmd !== "string"
	) {
		return false;
	}
...
export function isDockerInspectImageRecord(
	maybeImage: unknown,
): maybeImage is DockerInspectImageRecord {
...
export function normalizeDockerInspectImageRecord(
	image: DockerInspectImageRecord,
): InspectImagesItem {
	...
	const imageNameInfo: ImageNameInfo = parseDockerLikeImageName(
		image.RepoTags?.[0],
	);

	// Parse any environment variables defined for the image
	const environmentVariables = parseDockerLikeEnvironmentVariables(
		image.Config?.Env || [],
	);
	...
	const volumes = Object.entries(image.Config?.Volumes || {}).map<string>(
		([rawVolume]) => rawVolume,
	);
	...
	const architecture =
		image.Architecture?.toLowerCase() === "amd64"
			? "amd64"
			: image.Architecture?.toLowerCase() === "arm64"
				? "arm64"
				: undefined;

	// Parse and normalize the image OS
	const os =
		image.Os?.toLowerCase() === "linux"
			? "linux"
	...
	const isLocalImage = !(image.RepoDigests || []).some(
		(digest) => !digest.toLowerCase().startsWith("localhost/"),
	);
