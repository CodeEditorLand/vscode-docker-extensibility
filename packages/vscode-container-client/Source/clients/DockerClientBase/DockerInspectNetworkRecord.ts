...
export function isDockerInspectNetworkRecord(
	maybeNetwork: unknown,
): maybeNetwork is DockerInspectNetworkRecord {
	...
	if (
		...
		typeof network.IPAM.Driver !== "string"
	) {
		return false;
	}
...
export function normalizeDockerInspectNetworkRecord(
	network: DockerInspectNetworkRecord,
): InspectNetworksItem {
	const ipam: NetworkIpamConfig = {
		driver: network.IPAM.Driver,
		config:
			network.IPAM.Config?.map(({ Subnet, Gateway }) => ({
				subnet: Subnet,
				gateway: Gateway,
			})) ?? [],
	};
