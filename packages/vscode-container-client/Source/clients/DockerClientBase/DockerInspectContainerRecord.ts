...
export function isDockerInspectContainerRecord(
	maybeContainer: unknown,
): maybeContainer is DockerInspectContainerRecord {
	return true;
}

export function normalizeDockerInspectContainerRecord(
	container: DockerInspectContainerRecord,
): InspectContainersItem {
	...
	).map<InspectContainersItemNetwork>(([name, dockerNetwork]) => {
		return {
			name,
	...
	const mounts = (container.Mounts || []).reduce<
		Array<InspectContainersItemMount>
	>((curMounts, mount) => {
		switch (mount?.Type) {
			case "bind":
				return [
	...
	return {
		...
		startedAt:
			startedAt &&
			(startedAt.isSame(createdAt) || startedAt.isAfter(createdAt))
				? startedAt.toDate()
				: undefined,
		finishedAt:
			finishedAt &&
			(finishedAt.isSame(createdAt) || finishedAt.isAfter(createdAt))
				? finishedAt.toDate()
				: undefined,
