...
function withCommonOrchestratorArgs(
	options: CommonOrchestratorCommandOptions,
): CommandLineCurryFn {
	return composeArgs(
...
export class DockerComposeClient
	...
	implements IContainerOrchestratorClient
{
	/**
	...
	protected getUpCommandArgs(options: UpCommandOptions): CommandLineArgs {
		return composeArgs(
			...
			withNamedArg(
				"--scale",
				Object.entries(options.scale || {}).map(
	...
	protected getRestartCommandArgs(
	...
	protected getConfigCommandArgs(
	...
	public async config(
		options: ConfigCommandOptions,
	): Promise<PromiseCommandResponse<Array<ConfigItem>>> {
		return {
			command: this.commandName,
