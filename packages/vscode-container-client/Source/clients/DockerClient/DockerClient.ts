	...
	implements IContainersClient
{
	...
	private getListContextsCommandArgs(
		options: ListContextsCommandOptions,
	): CommandLineArgs {
		return composeArgs(withArg("context", "ls"), withDockerJsonFormatArg)();
	}
	...
	private getRemoveContextsCommandArgs(
	...
	private getUseContextCommandArgs(
	...
	private getInspectContextsCommandArgs(
	...
	private async parseInspectContextsCommandOutput(
		...
		try {
			return output
				.split("\n")
				.reduce<
					Array<InspectContextsItem>
				>((volumes, inspectString) => {
					if (!inspectString) {
						return volumes;
					}
	...
	override async inspectContexts(
		options: InspectContextsCommandOptions,
