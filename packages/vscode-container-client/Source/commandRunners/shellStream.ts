 ...
 */
export class ShellStreamCommandRunnerFactory<
	TOptions extends ShellStreamCommandRunnerOptions,
> implements ICommandRunnerFactory
{
	...
	public getCommandRunner(): CommandRunner {
		return async <T>(
			commandResponseLike:
				| Like<VoidCommandResponse>
				| Like<PromiseCommandResponse<T>>,
		) => {
	...
	private async *streamingCommandRunner<T>(
