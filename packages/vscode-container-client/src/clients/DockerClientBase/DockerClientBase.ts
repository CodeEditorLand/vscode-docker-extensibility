...
import {
	isDockerInspectContainerRecord,
	normalizeDockerInspectContainerRecord,
} from "./DockerInspectContainerRecord";
import {
	isDockerInspectImageRecord,
	normalizeDockerInspectImageRecord,
} from "./DockerInspectImageRecord";
import {
	isDockerInspectNetworkRecord,
	normalizeDockerInspectNetworkRecord,
} from "./DockerInspectNetworkRecord";
import {
	isDockerInspectVolumeRecord,
	normalizeDockerInspectVolumeRecord,
} from "./DockerInspectVolumeRecord";
import {
	isDockerListContainerRecord,
	normalizeDockerListContainerRecord,
} from "./DockerListContainerRecord";
import {
	isDockerListImageRecord,
	normalizeDockerListImageRecord,
} from "./DockerListImageRecord";
import {
	isDockerListNetworkRecord,
	normalizeDockerListNetworkRecord,
} from "./DockerListNetworkRecord";
...
import {
	parseListFilesCommandLinuxOutput,
	parseListFilesCommandWindowsOutput,
} from "./parseListFilesCommandOutput";
...
import {
	withDockerBooleanFilterArg,
	withDockerFilterArg,
} from "./withDockerFilterArg";
...
export abstract class DockerClientBase
	extends ConfigurableClient
	implements IContainersClient
{
	/**
	...
	protected getInfoCommandArgs(options: InfoCommandOptions): CommandLineArgs {
		return composeArgs(withArg("info"), withDockerJsonFormatArg)();
	}

	protected async parseInfoCommandOutput(
		output: string,
		strict: boolean,
	): Promise<InfoItem> {
	...
	): Promise<PromiseCommandResponse<InfoItem>> {
		return {
	 ...
	 */
	protected getVersionCommandArgs(
		options: VersionCommandOptions,
	): CommandLineArgs {
		return composeArgs(withArg("version"), withDockerJsonFormatArg)();
	}
	...
	async version(
		options: VersionCommandOptions,
	): Promise<PromiseCommandResponse<VersionItem>> {
		return {
			command: this.commandName,
	 ...
	 */
	protected getCheckInstallCommandArgs(
		options: CheckInstallCommandOptions,
	): CommandLineArgs {
		return composeArgs(withArg("-v"))();
	}
	...
	async checkInstall(
		options: CheckInstallCommandOptions,
	): Promise<PromiseCommandResponse<string>> {
		return {
			command: this.commandName,
	...
	protected getEventStreamCommandArgs(
		...
		return composeArgs(
			withArg("events"),
			withNamedArg("--since", options.since?.toString(), {
				shouldQuote: !(typeof options.since === "number"),
			}), // If it's numeric it should not be quoted
			withNamedArg("--until", options.until?.toString(), {
				shouldQuote: !(typeof options.until === "number"),
			}), // If it's numeric it should not be quoted
			withDockerLabelFilterArgs(options.labels),
			withDockerFilterArg(options.types?.map((type) => `type=${type}`)),
			withDockerFilterArg(
				options.events?.map((event) => `event=${event}`),
			),
		...
		for await (const line of lineReader) {
			...
			try {
				...
				yield {
					...
					actor: {
						id: item.Actor.ID,
						attributes: item.Actor.Attributes,
					},
	...
	async getEventStream(
		options: EventStreamCommandOptions,
	): Promise<GeneratorCommandResponse<EventItem>> {
		return {
			command: this.commandName,
			args: this.getEventStreamCommandArgs(options),
			parseStream: (output, strict) =>
				this.parseEventStreamCommandOutput(options, output, strict),
		};
	}
	...
	protected getLogoutCommandArgs(
		options: LogoutCommandOptions,
	): CommandLineArgs {
		return composeArgs(withArg("logout"), withArg(options.registry))();
	}
	 ...
	 */
	protected getBuildImageCommandArgs(
	 ...
	 */
	protected getListImagesCommandArgs(
	...
	async listImages(
		options: ListImagesCommandOptions,
	): Promise<PromiseCommandResponse<Array<ListImagesItem>>> {
		return {
			command: this.commandName,
			args: this.getListImagesCommandArgs(options),
			parse: (output, strict) =>
				this.parseListImagesCommandOutput(options, output, strict),
		};
	}
	...
	protected getRemoveImagesCommandArgs(
	...
	async removeImages(
		...
		return {
			...
			parse: (output, strict) =>
				this.parseRemoveImagesCommandOutput(options, output, strict),
		};
	}
	...
	protected getPushImageCommandArgs(
		options: PushImageCommandOptions,
	): CommandLineArgs {
		return composeArgs(
			withArg("image", "push"),
	...
	protected getPruneImagesCommandArgs(
	...
	protected parsePruneImagesCommandOutput(
		...
		const deletedImageLineRegex = /^(?:deleted:\s*sha256:\s*)(\w+)$/gim;
	...
	async pruneImages(
		options: PruneImagesCommandOptions,
	): Promise<PromiseCommandResponse<PruneImagesItem>> {
		return {
			command: this.commandName,
			args: this.getPruneImagesCommandArgs(options),
			parse: (output, strict) =>
				this.parsePruneImagesCommandOutput(options, output, strict),
		};
	}
	 ...
	 */
	protected getPullImageCommandArgs(
	...
	protected getTagImageCommandArgs(
			...
			return output
				...
				.reduce<Array<InspectImagesItem>>((images, inspectString) => {
					...
					try {
						...
						return [
							...images,
							normalizeDockerInspectImageRecord(inspect),
						];
					} catch (err) {
						if (strict) {
							throw err;
	...
	async inspectImages(
		options: InspectImagesCommandOptions,
	): Promise<PromiseCommandResponse<Array<InspectImagesItem>>> {
		return {
			command: this.commandName,
			args: this.getInspectImagesCommandArgs(options),
			parse: (output, strict) =>
				this.parseInspectImagesCommandOutput(options, output, strict),
		};
	}
	 ...
	 */
	protected getRunContainerCommandArgs(
	...
	async runContainer(
		options: RunContainerCommandOptions,
	): Promise<PromiseCommandResponse<string | undefined>> {
		return {
			command: this.commandName,
			args: this.getRunContainerCommandArgs(options),
			parse: (output, strict) =>
				this.parseRunContainerCommandOutput(options, output, strict),
		};
	}
	...
	protected getExecContainerCommandArgs(
	...
	async execContainer(
		options: ExecContainerCommandOptions,
	): Promise<GeneratorCommandResponse<string>> {
		return {
			command: this.commandName,
	...
	protected getListContainersCommandArgs(
		...
		return composeArgs(
			...
			withDockerFilterArg(
				options.imageAncestors?.map((id) => `ancestor=${id}`),
			),
			withDockerFilterArg(
				options.volumes?.map((volume) => `volume=${volume}`),
			),
			withDockerFilterArg(
				options.networks?.map((network) => `network=${network}`),
			),
			withDockerNoTruncArg,
			withDockerJsonFormatArg,
	...
	async listContainers(
		options: ListContainersCommandOptions,
	): Promise<PromiseCommandResponse<Array<ListContainersItem>>> {
		return {
			command: this.commandName,
			args: this.getListContainersCommandArgs(options),
			parse: (output, strict) =>
				this.parseListContainersCommandOutput(options, output, strict),
		};
	}
	...
	protected getStartContainersCommandArgs(
	...
	async startContainers(
		options: StartContainersCommandOptions,
	): Promise<PromiseCommandResponse<Array<string>>> {
		return {
			command: this.commandName,
			args: this.getStartContainersCommandArgs(options),
			parse: (output, strict) =>
				this.parseStartContainersCommandOutput(options, output, strict),
		};
	}
	...
	protected getRestartContainersCommandArgs(
	...
	async restartContainers(
		...
		return {
			...
			parse: (output, strict) =>
				this.parseRestartContainersCommandOutput(
					options,
					output,
					strict,
				),
	 ...
	 */
	protected getStopContainersCommandArgs(
	...
	async stopContainers(
		...
		return {
			...
			parse: (output, strict) =>
				this.parseStopContainersCommandOutput(options, output, strict),
		};
	}
	...
	protected getRemoveContainersCommandArgs(
	...
	async removeContainers(
		...
		return {
			...
			parse: (output, strict) =>
				this.parseRemoveContainersCommandOutput(
					options,
					output,
					strict,
				),
	...
	protected getPruneContainersCommandArgs(
		options: PruneContainersCommandOptions,
	): CommandLineArgs {
		return composeArgs(withArg("container", "prune"), withArg("--force"))();
	}
	...
	async pruneContainers(
		options: PruneContainersCommandOptions,
	): Promise<PromiseCommandResponse<PruneContainersItem>> {
		return {
			command: this.commandName,
			args: this.getPruneContainersCommandArgs(options),
			parse: (output, strict) =>
				this.parsePruneContainersCommandOutput(options, output, strict),
		};
	}
	...
	protected getStatsContainersCommandArgs(
	...
	async statsContainers(
		options: ContainersStatsCommandOptions,
	): Promise<PromiseCommandResponse<string>> {
		throw new CommandNotSupportedError(
			"statsContainers is not supported for this runtime",
		);
	}
	 ...
	 */
	protected getLogsForContainerCommandArgs(
	...
	protected async parseInspectContainersCommandOutput(
		...
		try {
			return output
				.split("\n")
				.reduce<
					Array<InspectContainersItem>
				>((containers, inspectString) => {
					if (!inspectString) {
						return containers;
					}
					...
					try {
						...
						return [
							...containers,
							normalizeDockerInspectContainerRecord(inspect),
						];
					} catch (err) {
						if (strict) {
							throw err;
	...
	async inspectContainers(
		...
		return {
			...
			parse: (output, strict) =>
				this.parseInspectContainersCommandOutput(
					options,
					output,
					strict,
				),
	...
	protected getCreateVolumeCommandArgs(
	...
	protected getListVolumesCommandArgs(
		...
		return composeArgs(
			...
			withDockerBooleanFilterArg("dangling", options.dangling),
			withDockerFilterArg(
				options.driver ? `driver=${options.driver}` : undefined,
			),
			withDockerLabelFilterArgs(options.labels),
			withDockerJsonFormatArg,
		)();
	...
	async listVolumes(
		options: ListVolumesCommandOptions,
	): Promise<PromiseCommandResponse<ListVolumeItem[]>> {
		return {
			command: this.commandName,
			args: this.getListVolumesCommandArgs(options),
			parse: (output, strict) =>
				this.parseListVolumesCommandOputput(options, output, strict),
		};
	}
	 ...
	 */
	protected getRemoveVolumesCommandArgs(
	...
	async removeVolumes(
		...
		return {
			...
			parse: (output, strict) =>
				this.parseRemoveVolumesCommandOutput(options, output, strict),
		};
	}
	...
	protected getPruneVolumesCommandArgs(
		options: PruneVolumesCommandOptions,
	): CommandLineArgs {
		return composeArgs(withArg("volume", "prune"), withArg("--force"))();
	}
	...
	async pruneVolumes(
		options: PruneVolumesCommandOptions,
	): Promise<PromiseCommandResponse<PruneVolumesItem>> {
		return {
			command: this.commandName,
			args: this.getPruneVolumesCommandArgs(options),
			parse: (output, strict) =>
				this.parsePruneVolumesCommandOutput(options, output, strict),
		};
	}
			...
			return output
				...
				.reduce<Array<InspectVolumesItem>>((volumes, inspectString) => {
					...
					try {
						...
						return [
							...volumes,
							normalizeDockerInspectVolumeRecord(inspect),
						];
					} catch (err) {
						if (strict) {
							throw err;
	...
	async inspectVolumes(
		options: InspectVolumesCommandOptions,
	): Promise<PromiseCommandResponse<Array<InspectVolumesItem>>> {
		return {
			command: this.commandName,
			args: this.getInspectVolumesCommandArgs(options),
			parse: (output, strict) =>
				this.parseInspectVolumesCommandOutput(options, output, strict),
		};
	}
	...
	protected getCreateNetworkCommandArgs(
		options: CreateNetworkCommandOptions,
	): CommandLineArgs {
		return composeArgs(
	...
	async listNetworks(
		options: ListNetworksCommandOptions,
	): Promise<PromiseCommandResponse<Array<ListNetworkItem>>> {
		return {
			command: this.commandName,
			args: this.getListNetworksCommandArgs(options),
			parse: (output, strict) =>
				this.parseListNetworksCommandOutput(options, output, strict),
		};
	}
	...
	protected getRemoveNetworksCommandArgs(
	...
	async removeNetworks(
		...
		return {
			...
			parse: (output, strict) =>
				this.parseRemoveNetworksCommandOutput(options, output, strict),
		};
	}
	...
	protected getPruneNetworksCommandArgs(
		options: PruneNetworksCommandOptions,
	): CommandLineArgs {
		return composeArgs(withArg("network", "prune"), withArg("--force"))();
	}
	...
	async pruneNetworks(
		options: PruneNetworksCommandOptions,
	): Promise<PromiseCommandResponse<PruneNetworksItem>> {
		return {
			command: this.commandName,
			args: this.getPruneNetworksCommandArgs(options),
			parse: (output, strict) =>
				this.parsePruneNetworksCommandOutput(options, output, strict),
		};
	}
	...
	protected async parseInspectNetworksCommandOutput(
		...
		try {
			return output
				.split("\n")
				.reduce<
					Array<InspectNetworksItem>
				>((networks, inspectString) => {
					if (!inspectString) {
						return networks;
					}
					...
					try {
						...
						return [
							...networks,
							normalizeDockerInspectNetworkRecord(inspect),
						];
					} catch (err) {
						if (strict) {
							throw err;
	...
	async inspectNetworks(
		options: InspectNetworksCommandOptions,
	): Promise<PromiseCommandResponse<InspectNetworksItem[]>> {
		return {
			command: this.commandName,
			args: this.getInspectNetworksCommandArgs(options),
			parse: (output, strict) =>
				this.parseInspectNetworksCommandOutput(options, output, strict),
		};
	}
	...
	async listContexts(
		options: ListContextsCommandOptions,
	): Promise<PromiseCommandResponse<ListContextItem[]>> {
		throw new CommandNotSupportedError(
			"listContexts is not supported for this runtime",
		);
	}
	...
	async removeContexts(
		options: RemoveContextsCommandOptions,
	): Promise<PromiseCommandResponse<string[]>> {
		throw new CommandNotSupportedError(
			"removeContexts is not supported for this runtime",
		);
	}
	...
	async useContext(
		options: UseContextCommandOptions,
	): Promise<VoidCommandResponse> {
		throw new CommandNotSupportedError(
			"useContext is not supported for this runtime",
		);
	}
	...
	async inspectContexts(
		options: InspectContextsCommandOptions,
	): Promise<PromiseCommandResponse<InspectContextsItem[]>> {
		throw new CommandNotSupportedError(
			"inspectContexts is not supported for this runtime",
		);
	}
	...
	protected getListFilesCommandArgs(
		...
		if (options.operatingSystem === "windows") {
			...
			const dirPath = options.path.endsWith("/")
				? options.path
				: options.path + "/";
			...
			command = [
				...
				"-c",
				{
					value: `stat -c '${LinuxStatArguments}' "${dirPath}"* || true && stat -c '${LinuxStatArguments}' "${dirPath}".* || true && stat -c '${LinuxStatArguments}' "${dirPath}".`,
	...
	async listFiles(
		options: ListFilesCommandOptions,
	): Promise<PromiseCommandResponse<ListFilesItem[]>> {
		return {
			command: this.commandName,
			args: this.getListFilesCommandArgs(options),
			parse: (output, strict) =>
				this.parseListFilesCommandOutput(options, output, strict),
		};
	}
	...
	protected getStatPathCommandArgs(
		...
		if (options.operatingSystem === "windows") {
			...
			command = [
				...
				"-c",
				{
					value: `stat -c '${LinuxStatArguments}' "${options.path}"`,
					quoting: ShellQuoting.Strong,
				},
			];
		}
	...
	async statPath(
		options: StatPathCommandOptions,
	): Promise<PromiseCommandResponse<StatPathItem | undefined>> {
		return {
			command: this.commandName,
			args: this.getStatPathCommandArgs(options),
			parse: (output, strict) =>
				this.parseStatPathCommandOutput(options, output, strict),
		};
	}
	...
	protected getReadFileCommandArgs(
		options: ReadFileCommandOptions,
	): CommandLineArgs {
		if (options.operatingSystem === "windows") {
			const command = ["cmd", "/D", "/S", "/C", `type "${options.path}"`];
	...
	async readFile(
		options: ReadFileCommandOptions,
	): Promise<GeneratorCommandResponse<Buffer>> {
		return {
			command: this.commandName,
	...
	protected getWriteFileCommandArgs(
		options: WriteFileCommandOptions,
	): CommandLineArgs {
		return composeArgs(
			withArg("cp"),
